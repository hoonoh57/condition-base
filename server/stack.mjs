...
export function predicates(rows, partition) {
...
export const featureJoin = 'FROM srb_derived.d_feat f JOIN srb_derived.d_ind i ON i.instrument_id=f.instrument_id AND i.trading_date=f.cond_date';
...
export async function evalStack(conn, input) {
  ...
  for (const row of [...input.rows].sort((a,b) => a.order_no-b.order_no)) {
    if (!row.enabled) { output.push({ ...row, n_survive: null, cut_pct: null, per_day: null,
      exec_pct: null, mfe_pass_pct: null, is_quality: row.cond_key === 'MFE_QUALITY' }); continue; }
    applied.push(row);
    const filter = predicates(applied, partition);
    const [[m]] = await conn.query(`
      ...
      ${featureJoin} WHERE ${filter.where}`,
    ...
    output.push({ ...row, n_survive: n,
      ...
      mfe_pass_pct: m.pass_pct == null ? null : Number(m.pass_pct),
