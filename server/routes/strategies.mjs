...
export default function strategies({ dbPool }) {
  const router = Router();
  router.get('/', route(async (_req, res) => {
    res.json(await connection(dbPool, async conn => {
  ...
  router.get('/conditions', route(async (_req, res) => res.json(await connection(dbPool, catalog))));
  router.post('/', route(async (req, res) => {
    const name = text(req.body.name, 80, 'name');
    res.status(201).json(await transaction(dbPool, async conn => {
  ...
  router.get('/:strategyId/versions', route(async (req, res) => {
    ...
    const [versions] = await dbPool.query(`SELECT v.*,m.part,m.data_asof,m.measured_at
  ...
  router.get('/versions/:versionId', route(async (req, res) => {
    ...
    res.json(await connection(dbPool, async conn => ({ version: await version(conn, versionId),
      rows: await loadRows(conn, versionId) })));
  }));
  router.get('/versions/:versionId/hypotheses', route(async (req, res) => {
    const [hypotheses] = await dbPool.query(
