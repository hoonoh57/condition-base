...
if (process.argv.includes('--dry-run')) await sqlCommand('readerPool', ['../sql/02_checks/p0_survivorship.sql']);
else {
  ...
  try {
    ...
    for (const statement of splitSql(sql)) {
      const [rows]=await db.readerPool.query(statement); results.push(rows);
    }
